...
class KakaoLoginAuthProvider with ChangeNotifier {
  ...
  issueAccessToken(context, String authCode) async {
    try {
      ...
      if (token.accessToken.isNotEmpty) {
        KakaoLoginAuthProvider()
            .kakaoUserData(context)
            .then((accountData) {
  ...
  Future<AccountData> kakaoUserData(context) async {
    var kakaoTalkAccount = new AccountData(); // 카카오톡 로그인 시, 로그인 유저 데이터 객체
